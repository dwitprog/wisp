...
{
    // CSS
    wp_enqueue_style('styles', get_stylesheet_directory_uri() . '/assets/min/css/styles.min.css', [], '_rgbld_18_5_2026');
    ...
    if (is_category() || is_archive() || is_single() || str_contains($_SERVER['REQUEST_URI'], 'components')) {
        wp_enqueue_style('posts', get_stylesheet_directory_uri() . '/assets/min/css/posts.min.css', [], '_rgbld_18_5_2026');
        ...
        if (file_exists($patch_page_style . get_the_ID() . '.min.css') && (is_single() || is_category() || is_archive())) {
            wp_enqueue_style('style-post-' . get_the_ID(), get_stylesheet_directory_uri() . '/assets/min/css/style-post-' . get_the_ID() . '.min.css', [], '_rgbld_18_5_2026');
        }
    } else {
        wp_enqueue_style('pages', get_stylesheet_directory_uri() . '/assets/min/css/pages.min.css', [], '_rgbld_18_5_2026');
        // Подключаем стили конкретной страницы
        $patch_page_style = get_stylesheet_directory() . '/assets/min/css/style-page-';
        if (file_exists($patch_page_style . get_the_ID() . '.min.css') && (!is_single() && !is_category() && !is_archive())) {
            wp_enqueue_style('style-page-' . get_the_ID(), get_stylesheet_directory_uri() . '/assets/min/css/style-page-' . get_the_ID() . '.min.css', [], '_rgbld_18_5_2026');
        }
    }
    ...
    foreach ($arr_display_styles as $d) {
        foreach ($arr_name_files_mobile_style as $f) {
            if (file_exists($patch_mobile_styles . $f . '-max-' . $d . '.css')) {
                wp_enqueue_style($f . '-max-' . $d, get_stylesheet_directory_uri() . '/assets/min/css/' . $f . '-max-' . $d . '.css', [], '_rgbld_18_5_2026', 'screen and (max-width:' . $d . 'px)');
    ...
    if (!empty($vendorsLibsWebpack)) {
        wp_enqueue_script('vendors', get_stylesheet_directory_uri() . '/assets/min/js/vendors.min.js', [], '_rgbld_18_5_2026', true);
    }
    wp_register_script('main', get_stylesheet_directory_uri() . '/assets/min/js/main.min.js', !empty($vendorsLibsWebpack) ? ['vendors'] : [], '_rgbld_18_5_2026', true);
    $rg_data = array(
...
{
    // CSS
    wp_enqueue_style('admin-styles', get_stylesheet_directory_uri() . '/assets/min/css/admin.min.css', [], '_rgbld_18_5_2026');
    wp_add_inline_style('admin-styles', st_get_font_face_styles());
    // JS
    wp_enqueue_script('admin-scripts', get_stylesheet_directory_uri() . '/assets/min/js/admin.min.js', [], '_rgbld_18_5_2026');
}
